...
class EmsAzure < EmsCloud
  ...
  # Called by the connect instance method.
  def self.raw_connect(access_key_id, secret_access_key, service = nil, region = nil, proxy_uri = nil)
    ...
    AzureConnection.raw_connect(access_key_id, secret_access_key, service, region, proxy_uri)
  end
  ...
  def connect(options = {})
    ...
    client_id = options[:user] || self.authentication_userid(options[:auth_type])
    client_key = options[:pass] || self.authentication_password(options[:auth_type])
    tenant_id = options[:tenant_id]

    self.class.raw_connect(client_id, client_key, options[:service], provider_region, options[:proxy_uri])
  end
